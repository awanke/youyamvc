...
public class CreateIdentifyingCode {
    ...
    public String getCookieKey(HttpServletRequest request){
        ...
        if(cookies == null) {
            ...
            for(int i$ = 0; i$ < len$; ++i$) {
                ...
                if(name.equals(IdentifyingCodeConstant.CHECK_CODE_SESSION_KEY)) {
    ...
    public void create(int storeType,String createTypeFlag,HttpServletRequest request, HttpServletResponse response) throws IOException {
        //1.在内存中创建一张图片
        ...
        if(storeType == IdentifyingCodeConstant.STORE_TYPE_MEMCACHE){
            //共享数据方案 多台服务器 不能放session 因为session没共享 放缓存即可
            //System.out.println("key1="+IdentifyingCodeConstant.CHECK_CODE_SESSION_KEY+getCookieKey(request));
            MemcachedClientUtils.resetCache(IdentifyingCodeConstant.CHECK_CODE_SESSION_KEY+getCookieKey(request),300,random);
        }else {
            request.getSession().setAttribute(IdentifyingCodeConstant.CHECK_CODE_SESSION_KEY, random);
        }
